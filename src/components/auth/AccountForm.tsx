...
import type { Account, Language, TrackedPolitician, WatchlistHandle } from '@/types'
import { POLITICAL_WATCHLIST } from '@/lib/nationalDiscourse'
...
type Step = 'profile' | 'tracking' | 'watchlist' | 'geography' | 'alerts' | 'review'
const STEPS: Step[] = ['profile', 'tracking', 'watchlist', 'geography', 'alerts', 'review']
const STEP_LABELS = { profile: 'Politician Profile', tracking: 'What to Track', watchlist: 'Social Watchlist', geography: 'Geography Scope', alerts: 'Alert Preferences', review: 'Review & Save' }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div style={{ background: 'var(--s1)', border: '1px solid var(--b2)', borderRadius: '14px', width: '100%', maxWidth: '680px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
          ...
          {step === 'watchlist' && (
            <WatchlistStep
              handles={form.watchlist_handles || []}
              onChange={handles => setField('watchlist_handles', handles)}
            />
          )}
...
// ─── Watchlist Step Component ─────────────────────────────────────────────────
const PRESET_JOURNALISTS = POLITICAL_WATCHLIST.journalists.map(h => ({
  id: `preset-j-${h}`, handle: h, display_name: '', platform: 'twitter' as const,
  category: 'journalist' as const, is_active: true, added_at: new Date().toISOString(),
}))
const PRESET_POLITICIANS = POLITICAL_WATCHLIST.politicians_tracked.map(h => ({
  id: `preset-p-${h}`, handle: h, display_name: '', platform: 'twitter' as const,
  category: 'politician' as const, is_active: true, added_at: new Date().toISOString(),
}))

function WatchlistStep({ handles, onChange }: {
  handles: WatchlistHandle[]
  onChange: (handles: WatchlistHandle[]) => void
}) {
  const [newHandle, setNewHandle]   = useState('')
  const [newName, setNewName]       = useState('')
  const [newPlatform, setNewPlatform] = useState<'twitter' | 'bluesky' | 'instagram' | 'all'>('twitter')
  const [newCategory, setNewCategory] = useState<'journalist' | 'politician' | 'commentator' | 'influencer' | 'custom'>('journalist')
  const [presetTab, setPresetTab]   = useState<'journalists' | 'politicians'>('journalists')
  ...
  const T0 = '#edf0f8', T1 = '#c8d0e0', T2 = '#8892a4', T3 = '#545f78'
  const CARD2 = '#161d2c'

  const catColor = (cat: string) => ({
    journalist: BLUE, politician: ACC, commentator: '#7c6dfa',
    influencer: '#e1306c', custom: GREEN,
  }[cat] || T2)

  function addHandle() {
    const h = newHandle.trim()
    if (!h) return
    if (handles.some(x => x.handle.toLowerCase() === h.toLowerCase())) return
    onChange([...handles, {
      id: `custom-${Date.now()}`, handle: h.startsWith('@') ? h : `@${h}`,
      display_name: newName.trim() || undefined, platform: newPlatform,
      category: newCategory, is_active: true, added_at: new Date().toISOString(),
    }])
    setNewHandle(''); setNewName('')
  }

  function addPreset(preset: WatchlistHandle) {
    if (handles.some(x => x.handle === preset.handle)) return
    onChange([...handles, preset])
  }

  function addAllPresets(presets: WatchlistHandle[]) {
    const existing = new Set(handles.map(h => h.handle))
    const newOnes = presets.filter(p => !existing.has(p.handle))
    onChange([...handles, ...newOnes])
  }

  function toggleHandle(id: string) {
    onChange(handles.map(h => h.id === id ? { ...h, is_active: !h.is_active } : h))
  }
  ...
  const filtered = filterCat === 'all' ? handles : handles.filter(h => h.category === filterCat)
  const cats = ['all', 'journalist', 'politician', 'commentator', 'influencer', 'custom']

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
      {/* Info banner */}
      <div style={{ padding: '10px 14px', background: 'rgba(61,142,240,0.08)', border: '1px solid rgba(61,142,240,0.2)', borderRadius: '8px' }}>
        <div style={{ fontFamily: mono, fontSize: '8px', color: BLUE, marginBottom: '3px' }}>SOCIAL MEDIA WATCHLIST</div>
        <div style={{ fontFamily: mono, fontSize: '8px', color: T2, lineHeight: 1.8 }}>
          Track specific Twitter, Bluesky, and Instagram handles. Every post from these accounts is automatically ingested.<br />
          Add your own handles below or pick from our curated list of {PRESET_JOURNALISTS.length + PRESET_POLITICIANS.length} Indian political journalists and politicians.
        </div>
      </div>

      {/* Add custom handle */}
      <div style={{ background: CARD2, border: `1px solid ${BORDER}`, borderRadius: '10px', padding: '14px' }}>
        <div style={{ fontFamily: mono, fontSize: '8px', color: T2, letterSpacing: '1px', marginBottom: '10px' }}>ADD HANDLE MANUALLY</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '6px', marginBottom: '8px' }}>
          <input value={newHandle} onChange={e => setNewHandle(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && addHandle()}
            placeholder="@handle or handle.bsky.social" />
          <input value={newName} onChange={e => setNewName(e.target.value)}
            placeholder="Display name (optional)" />
          <button onClick={addHandle}
            style={{ padding: '8px 14px', background: 'rgba(249,115,22,0.15)', border: '1px solid rgba(249,115,22,0.3)', borderRadius: '6px', color: ACC, fontFamily: mono, fontSize: '9px', cursor: 'pointer', whiteSpace: 'nowrap' }}>
            + ADD
          </button>
        </div>
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          {(['twitter', 'bluesky', 'instagram', 'all'] as const).map(p => (
            <button key={p} onClick={() => setNewPlatform(p)}
              style={{ padding: '3px 10px', borderRadius: '20px', border: `1px solid ${newPlatform === p ? BLUE : BORDER}`, background: newPlatform === p ? BLUE + '15' : 'transparent', color: newPlatform === p ? BLUE : T3, fontFamily: mono, fontSize: '7px', cursor: 'pointer', textTransform: 'uppercase' }}>
              {p}
            </button>
          ))}
          <div style={{ width: '1px', background: BORDER }} />
          {(['journalist', 'politician', 'commentator', 'influencer', 'custom'] as const).map(c => (
            <button key={c} onClick={() => setNewCategory(c)}
              style={{ padding: '3px 10px', borderRadius: '20px', border: `1px solid ${newCategory === c ? catColor(c) : BORDER}`, background: newCategory === c ? catColor(c) + '15' : 'transparent', color: newCategory === c ? catColor(c) : T3, fontFamily: mono, fontSize: '7px', cursor: 'pointer', textTransform: 'uppercase' }}>
              {c}
            </button>
          ))}
        </div>
      ...
      {/* Preset picker */}
      <div style={{ background: CARD2, border: `1px solid ${BORDER}`, borderRadius: '10px', overflow: 'hidden' }}>
        <div style={{ padding: '10px 14px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontFamily: mono, fontSize: '8px', color: T2, letterSpacing: '1px', flex: 1 }}>PRESET HANDLES — VERIFIED INDIAN POLITICAL ACCOUNTS</span>
          <div style={{ display: 'flex', gap: '4px' }}>
            {(['journalists', 'politicians'] as const).map(t => (
              <button key={t} onClick={() => setPresetTab(t)}
                style={{ padding: '3px 10px', borderRadius: '4px', border: `1px solid ${presetTab === t ? BLUE : BORDER}`, background: presetTab === t ? BLUE + '15' : 'transparent', color: presetTab === t ? BLUE : T3, fontFamily: mono, fontSize: '7px', cursor: 'pointer', textTransform: 'uppercase' }}>
                {t} ({t === 'journalists' ? PRESET_JOURNALISTS.length : PRESET_POLITICIANS.length})
              </button>
            ))}
          </div>
          <button
            onClick={() => addAllPresets(presetTab === 'journalists' ? PRESET_JOURNALISTS : PRESET_POLITICIANS)}
            style={{ padding: '4px 10px', borderRadius: '4px', border: `1px solid ${GREEN}40`, background: GREEN + '10', color: GREEN, fontFamily: mono, fontSize: '7px', cursor: 'pointer' }}>
            + ADD ALL
          </button>
        </div>
        <div style={{ padding: '10px', display: 'flex', gap: '4px', flexWrap: 'wrap', maxHeight: '180px', overflowY: 'auto' }}>
          {(presetTab === 'journalists' ? PRESET_JOURNALISTS : PRESET_POLITICIANS).map(p => {
            const added = handles.some(h => h.handle === p.handle)
            const c = presetTab === 'journalists' ? BLUE : ACC
            return (
              <button key={p.handle} onClick={() => !added && addPreset(p)} disabled={added}
                style={{ padding: '3px 8px', borderRadius: '20px', border: `1px solid ${added ? BORDER : c + '30'}`, background: added ? c + '08' : c + '08', color: added ? T3 : c, fontFamily: mono, fontSize: '7px', cursor: added ? 'default' : 'pointer', opacity: added ? 0.5 : 1, transition: 'all .15s' }}>
                {added ? '✓ ' : ''}{p.handle}
              </button>
            )
          })}
      ...
      {handles.length > 0 && (
        <div>
          <div style={{ display: 'flex', gap: '4px', marginBottom: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontFamily: mono, fontSize: '8px', color: T3, marginRight: '4px' }}>YOUR WATCHLIST ({handles.length})</span>
            {cats.map(c => {
              const count = c === 'all' ? handles.length : handles.filter(h => h.category === c).length
              if (count === 0 && c !== 'all') return null
              return (
                <button key={c} onClick={() => setFilterCat(c)}
                  style={{ padding: '2px 8px', borderRadius: '20px', border: `1px solid ${filterCat === c ? catColor(c) : BORDER}`, background: filterCat === c ? catColor(c) + '15' : 'transparent', color: filterCat === c ? catColor(c) : T3, fontFamily: mono, fontSize: '7px', cursor: 'pointer', textTransform: 'uppercase' }}>
                  {c} ({count})
                </button>
              )
            })}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', maxHeight: '260px', overflowY: 'auto' }}>
            {filtered.map(h => {
              ...
              return (
                <div key={h.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 10px', background: CARD2, border: `1px solid ${BORDER}`, borderRadius: '7px', opacity: h.is_active ? 1 : 0.5 }}>
                  <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: h.is_active ? GREEN : T3, flexShrink: 0, cursor: 'pointer' }}
                    onClick={() => toggleHandle(h.id)} title={h.is_active ? 'Click to pause' : 'Click to activate'} />
                  <span style={{ fontFamily: mono, fontSize: '9px', color: T0, flex: 1 }}>{h.handle}</span>
                  {h.display_name && <span style={{ fontFamily: mono, fontSize: '8px', color: T2 }}>{h.display_name}</span>}
                  <span style={{ fontFamily: mono, fontSize: '7px', padding: '1px 6px', background: c + '12', border: `1px solid ${c}20`, borderRadius: '10px', color: c }}>{h.category}</span>
                  <span style={{ fontFamily: mono, fontSize: '7px', color: T3 }}>{h.platform}</span>
                  <button onClick={() => removeHandle(h.id)}
                    style={{ background: 'none', border: 'none', color: T3, cursor: 'pointer', fontSize: '14px', lineHeight: 1, padding: '0 2px' }}
                    onMouseEnter={e => { (e.target as HTMLElement).style.color = '#f03e3e' }}
                    onMouseLeave={e => { (e.target as HTMLElement).style.color = T3 }}>×</button>
                </div>
              )
            })}
          </div>
