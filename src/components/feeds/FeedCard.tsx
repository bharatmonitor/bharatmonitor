...
interface Props { item: FeedItem; onClick?: () => void; selected?: boolean }

// Stable selectors — per-field to avoid full-store re-renders
const selectOpenVideo = (s: ReturnType<typeof useDashboardStore.getState>) => s.openVideo
const selectOpenItem  = (s: ReturnType<typeof useDashboardStore.getState>) => s.openItem

export default function FeedCard({ item, onClick, selected }: Props) {
  const openVideo = useDashboardStore(selectOpenVideo)
  const openItem  = useDashboardStore(selectOpenItem)
  const isYT = item.platform === 'youtube'

  function handleClick() {
    // YouTube: open in-portal video modal (already in place)
    if (isYT && item.youtube_id) {
      openVideo(item.youtube_id, item.headline)
      return
    }
    // All other platforms: open the in-portal detail panel
    // Users stay on BharatMonitor — the panel has ONE explicit "Open source ↗" link
    openItem(item)
  }
    ...
    <div
      ...
      {item.contradiction && (
        <div style={{
          ...
          background: item.contradiction.contradiction_type === 'flip' ? 'rgba(240,62,62,0.07)' : 'rgba(245,166,35,0.07)',
          border: `1px solid ${item.contradiction.contradiction_type === 'flip' ? 'rgba(240,62,62,0.18)' : 'rgba(245,166,35,0.18)'}`,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '2px' }}>
            <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '7px', color: item.contradiction.contradiction_type === 'flip' ? 'var(--red)' : 'var(--yel)' }}>
              {item.contradiction.contradiction_type === 'flip' ? 'POSITION FLIP' : item.contradiction.contradiction_type === 'vote_record' ? 'VOTE RECORD' : 'CONTRADICTION'}
            </span>
      ...
      <div style={{ display: 'flex', gap: '3px', flexWrap: 'wrap' }}>
        ...
        {/* Google-sourced X posts */}
        {item.id.startsWith('gx-') && (
          <span style={{ fontSize: '7px', padding: '1px 5px', borderRadius: '2px', fontFamily: 'IBM Plex Mono, monospace', background: 'rgba(29,155,240,0.08)', color: '#1d9bf0', border: '1px solid rgba(29,155,240,0.22)', display: 'flex', alignItems: 'center', gap: '2px' }}>
            𝔾 via Google
          </span>
        )}
      </div>
