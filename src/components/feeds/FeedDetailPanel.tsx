...
const PLAT_ICON: Record<string, string> = {
  twitter: 'X', instagram: 'IG', facebook: 'f', whatsapp: 'W',
  youtube: 'YT', news: 'N', reddit: 'R',
}
...
export default function FeedDetailPanel({ trackedPoliticianNames = [], accountId = '' }: Props) {
  ...
  const [liveContradiction, setLiveContradiction] = useState<any>(null)
  const [checkRan, setCheckRan] = useState(false)

  useEffect(() => {
    setLiveContradiction(null)
    setCheckRan(false)
  }, [item?.id])
  ...
  const platIcon  = PLAT_ICON[item.platform]  ?? 'N'
  ...
  const canCheck = trackedPoliticianNames.length > 0 && !!accountId

  function handleCheck() {
    if (!canCheck || !item) return
    setCheckRan(true)
    checkItem.mutate(item, {
      onSuccess: (result) => { if (result) setLiveContradiction(result) },
    })
  }

  // Resolved contradiction — prefer live check result, fall back to item's stored contradiction
  const contra: any = liveContradiction || item.contradiction || null

  const ctype   = contra ? getField(contra, 'contradiction_type', 'contradictionType') : ''
  const cscore  = contra ? (getField(contra, 'contradiction_score', 'contradictionScore') || 0) : 0
  const hdate   = contra ? String(getField(contra, 'historical_date', 'historicalDate')).substring(0, 7) : ''
  const hquote  = contra ? getField(contra, 'historical_quote', 'historicalQuote') : ''
  const hsource = contra ? getField(contra, 'historical_source', 'historicalSource') : ''
  const reasoning = contra ? getField(contra, 'reasoning') : ''
  const clabel  = ctype === 'flip' ? 'POSITION FLIP DETECTED'
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            fontSize: '11px', fontWeight: 700, color: platColor, flexShrink: 0,
            fontFamily: mono,
          }}>
            {platIcon}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: mono, fontSize: '10px', color: platColor, fontWeight: 600 }}>
              {item.platform.toUpperCase()}
              {isGoogleXPost && <span style={{ color: '#8892a4', fontWeight: 400 }}> · via Google</span>}
            </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
          ...
          <h2 style={{ fontSize: '15px', fontWeight: 600, color: 'var(--t0)', lineHeight: 1.5, margin: '0 0 12px 0' }}>
            {item.headline}
          </h2>
          ...
          {canCheck && !contra && (
            <div style={{ marginBottom: '14px' }}>
              ...
              >
                {checkItem.isPending ? 'SCANNING 5-YEAR RECORD...' : checkRan ? 'NO CONTRADICTION FOUND' : 'CHECK FOR CONTRADICTIONS (5-YEAR RECORD)'}
              </button>
            </div>
          )}
          ...
          {contra && (
            <div style={{ padding: '14px', borderRadius: '10px', marginBottom: '14px', background: 'rgba(245,166,35,0.06)', border: '1px solid rgba(245,166,35,0.25)' }}>
              <div style={{ fontFamily: mono, fontSize: '8px', color: '#f5a623', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span>{clabel}</span>
                <span style={{ marginLeft: 'auto', padding: '2px 6px', borderRadius: '3px', background: 'rgba(245,166,35,0.15)', fontWeight: 700 }}>{cscore}%</span>
              </div>
              <div style={{ fontFamily: mono, fontSize: '7px', color: 'var(--t3)', marginBottom: '5px' }}>HISTORICAL RECORD:</div>
              <div style={{ fontSize: '11px', color: 'var(--t1)', lineHeight: 1.6, marginBottom: '5px' }}>
                {hdate && <span style={{ color: 'var(--t3)' }}>{hdate}: </span>}
                {hquote}
              </div>
              {hsource ? (
                <div style={{ fontFamily: mono, fontSize: '8px', color: 'var(--t3)' }}>— {hsource}</div>
              ) : null}
              {reasoning ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px', marginBottom: '14px' }}>
              {[
                ...
                <div key={s.label} style={{ background: 'var(--s2)', border: '1px solid var(--b0)', borderRadius: '7px', padding: '9px 10px', textAlign: 'center' }}>
                  <div style={{ fontFamily: mono, fontSize: '7px', color: 'var(--t3)', marginBottom: '3px' }}>{s.label}</div>
                  <div style={{ fontFamily: mono, fontSize: '14px', fontWeight: 700, color: 'var(--t0)' }}>
                    {s.value > 999999 ? `${(s.value / 1000000).toFixed(1)}M`
                      : s.value > 999 ? `${(s.value / 1000).toFixed(0)}K`
                      : s.value || '—'}
                  </div>
                </div>
              ))}
          ...
          {item.opp_risk !== undefined && item.opp_risk > 0 && (
            <div style={{ marginBottom: '14px' }}>
              <div style={{ fontFamily: mono, fontSize: '7px', color: 'var(--t3)', marginBottom: '5px' }}>OPPOSITION ATTACK RISK</div>
              <div style={{ height: '6px', background: 'var(--s3)', borderRadius: '3px', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${item.opp_risk}%`, background: item.opp_risk > 70 ? '#f03e3e' : item.opp_risk > 40 ? '#f5a623' : '#22d3a0', borderRadius: '3px' }} />
              </div>
        ...
        <div style={{
          ...
          {item.url ? (
            ...
            <div style={{ flex: 1, fontFamily: mono, fontSize: '8px', color: 'var(--t3)', textAlign: 'center' }}>No source URL</div>
          )}
          <button onClick={closeItem} style={{
