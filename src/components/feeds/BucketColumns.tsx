...
export default function BucketColumns({ feed, accountId }: { feed: FeedItem[]; accountId: string }) {
  ...
  // Update counts in a useEffect (not inside useMemo — that causes setState-during-render)
  useEffect(() => {
    setAllCounts({ red: byBucket.red.length, yellow: byBucket.yellow.length, blue: byBucket.blue.length, silver: byBucket.silver.length })
  }, [byBucket, setAllCounts])
