...
import NationalDiscoursePanel from '@/components/dashboard/NationalDiscoursePanel'
...
function SentimentPanel({ feed }: { feed: FeedItem[] }) {
  ...
  const sentScore = Math.round((pos / total) * 100)
  const sentColor = sentScore >= 55 ? '#22d3a0' : sentScore <= 45 ? '#f03e3e' : '#f5a623'

  return (
    <Section title="SENTIMENT PULSE" badge={`${feed.length} ITEMS`}>
      {/* Overall donut + stats */}
      <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'12px', paddingBottom:'10px', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
        {/* Mini gauge */}
        <div style={{ position:'relative', flexShrink:0 }}>
          <svg width={56} height={56} viewBox="0 0 56 56">
            <circle cx={28} cy={28} r={22} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth={7}/>
            <circle cx={28} cy={28} r={22} fill="none" stroke={sentColor} strokeWidth={7}
              strokeDasharray={`${(sentScore/100)*138} 138`} strokeLinecap="round"
              transform="rotate(-90 28 28)" style={{ transition:'stroke-dasharray .6s ease' }}/>
            <text x={28} y={32} textAnchor="middle" fill={sentColor} fontSize={11} fontFamily='"IBM Plex Mono",monospace' fontWeight={700}>{sentScore}</text>
          </svg>
        </div>
        <div style={{ flex:1 }}>
          {[['Positive', pos, '#22d3a0'], ['Negative', neg, '#f03e3e'], ['Neutral', neu, '#545f78']].map(([label, count, color]) => (
            <div key={label as string} style={{ display:'flex', alignItems:'center', gap:'5px', marginBottom:'4px' }}>
              <div style={{ width:'6px', height:'6px', borderRadius:'50%', background:color as string, flexShrink:0 }} />
              <span style={{ fontFamily:'"IBM Plex Mono",monospace', fontSize:'8px', color:'#c8d0e0', flex:1 }}>{label}</span>
              <span style={{ fontFamily:'"IBM Plex Mono",monospace', fontSize:'8px', color:color as string, fontWeight:700 }}>{count}</span>
              <span style={{ fontFamily:'"IBM Plex Mono",monospace', fontSize:'7px', color:'#545f78' }}>{Math.round((count as number)/total*100)}%</span>
            </div>
          ))}
        </div>
      </div>

      {/* Per-platform sentiment — side by side */}
      <div style={{ fontFamily:'"IBM Plex Mono",monospace', fontSize:'7px', color:'#545f78', letterSpacing:'1px', marginBottom:'6px' }}>BY PLATFORM</div>
      {byPlatform.length === 0 ? (
        <div style={{ fontFamily:'"IBM Plex Mono",monospace', fontSize:'8px', color:'#545f78' }}>Fetching platform data…</div>
      ) : (
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'5px' }}>
          {byPlatform.map(([plat, s]) => {
            const c = PLAT_COLORS_S[plat] || '#8892a4'
            const pp = Math.round(s.pos/Math.max(s.total,1)*100)
            const np = Math.round(s.neg/Math.max(s.total,1)*100)
            const dominant = pp > np ? 'pos' : np > pp ? 'neg' : 'neu'
            const domColor = dominant === 'pos' ? '#22d3a0' : dominant === 'neg' ? '#f03e3e' : '#545f78'
            return (
              <div key={plat} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'7px', padding:'7px 8px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'5px' }}>
                  <span style={{ fontFamily:'"IBM Plex Mono",monospace', fontSize:'7px', color:c, fontWeight:700, letterSpacing:'0.5px' }}>{plat.toUpperCase().slice(0,7)}</span>
                  <span style={{ fontFamily:'"IBM Plex Mono",monospace', fontSize:'7px', color:'#545f78' }}>{s.total}</span>
                </div>
                {/* Stacked sentiment bar */}
                <div style={{ height:'4px', background:'rgba(255,255,255,0.05)', borderRadius:'2px', overflow:'hidden', display:'flex', marginBottom:'4px' }}>
                  <div style={{ width:`${pp}%`, height:'100%', background:'#22d3a0' }}/>
                  <div style={{ width:`${np}%`, height:'100%', background:'#f03e3e' }}/>
                  <div style={{ flex:1, height:'100%', background:'rgba(255,255,255,0.06)' }}/>
                </div>
...
export default function Sidebar({ pulse, competitors, schemes, issueOwnership, account, brief, feed }: Props) {
