...
const selectActivePlatform = (s: ReturnType<typeof useDashboardStore.getState>) => s.activePlatform
...
// Wrapper that memoizes the values array so Sparkline's useEffect doesn't fire
// on every parent render (t.data_points.map() always creates a new array reference)
function SparklineMemo({ dataPoints, color }: { dataPoints: { value: number }[]; color: string }) {
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const data = useMemo(() => dataPoints.map(d => d.value), [dataPoints.map(d => d.value).join(',')])
  return <div style={{ marginTop: '5px' }}><Sparkline data={data} color={color} /></div>
}

interface Props { trends: TrendMetric[] }

export default function TrendStrip({ trends }: Props) {
  const activePlatform = useDashboardStore(selectActivePlatform)
      ...
      {trends.map(t => {
        ...
        return (
          <div key={t.id} style={{ background: 'var(--s2)', border: '1px solid var(--b1)', borderRadius: '7px', padding: '7px 9px', minWidth: '110px', flexShrink: 0, transition: 'border-color .15s', cursor: 'default' }}
            ...
            <SparklineMemo dataPoints={t.data_points} color={cfg.color} />
          </div>
        )
      })}
    </div>
