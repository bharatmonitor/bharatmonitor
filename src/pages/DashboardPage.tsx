...
import { syncCredentialToSupabase } from '@/lib/accounts'
    ...
    triggerEdgeIngest(accountId, acc.politician_name, acc.keywords)
      .then(result => {
        if (result.inserted > 0) {
          ...
          // Record quota usage
          const newQuota = recordSearch(accountId, { news: Math.min(result.inserted || 0, 5), social: result.inserted || 0 })
  ...
  return (
    <ErrorBoundary>
      <div style={{ height:'100vh', background:'var(--bg)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        <CommandBar account={account!} kpis={kpis} onSearchClick={() => setShowSearch(true)} onSettingsClick={() => setShowSettings(true)} onAnalyticsClick={() => setShowAnalytics(true)} onSearchNow={handleSearchNow} isSearching={!ingestDone && feedLoading} fuelLevel={fuelLevel} />
        <AIRibbon brief={tickerBrief} />
        <BucketNav />
        <PlatformFilter />
        {trends.length > 0 && <TrendStrip trends={trends} />}

        <div className="dashboard-grid" style={{ display:'grid', gridTemplateColumns:'1fr 300px', flex:1, overflow:'hidden', minHeight:0 }}>
          <div style={{ overflow:'hidden', display:'flex', flexDirection:'column', minHeight:0 }}>
            {feedLoading && dbFeed.length === 0 && feed.length === 0 ? (
