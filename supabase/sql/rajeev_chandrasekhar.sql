-- ============================================================================
-- BharatMonitor — Rajeev Chandrasekhar (Kerala BJP President & MLA, Nemom)
-- Full detailed account, schema-matched. Re-runnable (ON CONFLICT).
-- Context: Kerala 2026 Assembly election (4 May 2026) — UDF/Congress landslide
-- (102 seats) ousted the LDF (35). Chandrasekhar WON Nemom (defeated V.Sivankutty),
-- and is Kerala BJP State President. Opposition-to-monitor is now Congress/UDF.
-- ============================================================================
...
insert into public.accounts (
  ...
  'politician', 'Rajeev Chandrasekhar', 'RC', 'BJP', 'Kerala BJP President & MLA (Nemom)',
  ...
  'rajeev@bharatmonitor.online', 'demo@1234', 'Rajeev Chandrasekhar', 'user', 'elections',
  now(), now()
)
on conflict (id) do update set
  ...
  tier = excluded.tier,
  login_email = excluded.login_email, login_password = excluded.login_password,
  login_name = excluded.login_name, login_role = excluded.login_role, login_tier = excluded.login_tier,
  is_active = true, updated_at = now();

-- Verify
SELECT id, politician_name, login_email, login_password, login_tier FROM accounts
WHERE id = 'BM-2026-RCKERALA';
